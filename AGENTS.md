...
- 托盘菜单相关样式和交互逻辑应集中在 `Models\Tray\` 目录下，`FormMain.cs` 只保留业务回调和状态同步，避免主窗体继续堆积自绘菜单细节。
- 托盘菜单按职责分层：入口协调放在 `TrayBrightnessMenu`，控件组装和状态刷新放在 `TrayBrightnessMenuView`，DPI/窗口定位放在 `TrayNativeWindowHelper`，布局数值放在 `TrayLayout`，自绘控件各自独立文件。后续调整不要重新把这些职责揉回一个大类。
- `TrayLayout` 中 1080/100% 的 compact 布局和 4K/高 DPI 的 regular 布局应分开维护。修某个分辨率问题时优先只改对应工厂方法，避免 1080 和 4K 互相回归。
- 托盘菜单中的可点击菜单项不要混用自绘弹窗背景和独立 WinForms 按钮控件。首次打开时子控件句柄、透明背景和系统默认绘制可能出现首帧边框、文字重影或样式不一致；优先在同一个自绘面板里统一绘制菜单项、hover 背景和点击命中。
- 自绘复选框在低 DPI compact 布局下不能把控件宽度缩到绘制框同宽；需要保留最小绘制余量，避免首次打开时边线被裁切成凹陷或系统默认样式的错觉。
